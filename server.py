...
def get_key(names: tuple[str, ...]) -> str:
    for name in names:
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path.rstrip("/") == "/api/status":
            status = {
                name: bool(get_key(cfg["key_names"]))
                for name, cfg in PROVIDERS.items()
            }
            self._json(200, {"providers": status})
            return
        return super().do_GET()
    ...
    def do_POST(self):
        ...
        if provider not in PROVIDERS:
            self._json(400, {"error": "unknown provider: " + provider})
            return
        ...
        if not key:
            self._json(
                500,
                {"error": "No API key for %s. Set %s in .env" % (provider, cfg["key_names"][0])},
            )
            return

        if cfg["logprobs"] and body.get("logprobs") is True and provider == "together":
            n = body.get("top_logprobs") or 5
            body["logprobs"] = int(n)
        ...
        headers = {
            ...
            "User-Agent": "LLM-Teaching-Tool/0.2 (+https://github.com/thamam/llm-teaching-tool)",
        ...
        try:
            ...
            if "1010" in text or err.code == 403:
                payload = json.dumps({
                    "error": (
                        "Cloudflare 1010 from %s. The proxy User-Agent was blocked, "
                        "or your IP is flagged. Retry after pulling the UA fix; "
                        "if it persists, use OpenRouter/Together or curl from your machine."
                    ) % provider
                }).encode()
